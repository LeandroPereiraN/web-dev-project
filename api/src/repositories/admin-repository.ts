...
import { ModerationActionNotFoundError } from "../plugins/errors.js";
...
export type ModerationActionCreatePayload = Static<
  typeof ModerationActionCreateInput
> & { admin_id: number };
...
class AdminRepository {
  ...
  static async getReportedSellers(
    page: number,
    limit: number,
    client?: PoolClient
  ): Promise<{
    sellers: ReportedSellerType[];
    total: number;
    page: number;
    limit: number;
  }> {
  ...
  static async createModerationAction(
    payload: ModerationActionCreatePayload,
    client?: PoolClient
  ): Promise<ModerationActionType> {
    const sql = `
  ...
  static async createAdminNotification(
    ...
    await this.runQuery(
      sql,
      [sellerId, moderationActionId, title, message],
      client
    );
  }
}
